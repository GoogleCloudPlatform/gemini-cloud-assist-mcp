 ...
 *
 * This tool provides a comprehensive, human-readable answer
 * synthesized by the backend agent, not raw resource data.
 *
 * VERY IMPORTANT: The resource type is required to be included in this query so
 * the agent knows where to begin searching. If the resource type is NOT KNOWN
 * AND CANNOT BE INFERRED, you must first ask a follow-up question to identify
 * the resource type then pass that into this tool.
 *
 * - Due to the powerful nature of this tool, simple queries such as "list my VMs" which can be
 * solved by one gcloud command should be handled by the gcloud shell tool. HOWEVER, if the
 * gcloud shell tool fails for retrieving resources, you may use this tool.
 *
 * @param {string} content [REQUIRED] The user's full natural language question
 * or request regarding GCP resources. This should be detailed, INCLUDE the
 * resource type and capture the exact intent of the user. The backend agent will interpret this query.
