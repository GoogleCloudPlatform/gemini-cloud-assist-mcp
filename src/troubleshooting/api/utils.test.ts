...
const realisticBasePayload: any = {
...
describe('API Utils', () => {
    it('createInitialInvestigation should create a valid payload.', () => {
        ...
        expect(initialPayload.observations['user.input.text'].relevantResources).toHaveLength(1);
    });
    ...
    it('getRevisionWithNewObservation should append text and resources.', () => {
        ...
        const originalUserObsCount = Object.values(realisticBasePayload.observations).filter((o: any) => o.observerType === 'OBSERVER_TYPE_USER').length;
        const newUserObsCount = Object.values(revision1!.snapshot.observations).filter((o: any) => o.observerType === 'OBSERVER_TYPE_USER').length;
    ...
    it('getRevisionWithNewObservation should handle back-to-back calls.', () => {
        ...
        const originalUserObsCount = Object.values(realisticBasePayload.observations).filter((o: any) => o.observerType === 'OBSERVER_TYPE_USER').length;
        const finalUserObsCount = Object.values(revision2!.snapshot.observations).filter((o: any) => o.observerType === 'OBSERVER_TYPE_USER').length;
    ...
    describe('Edge Cases', () => {
        ...
        it('Handle payload with no observations property.', () => {
            const noObsPayload: any = { title: 'Test' };
...
describe('InvestigationPath', () => {
    it('Constructor and basic getters should work.', () => {
        ...
        expect(path.getInvestigationName()).toBe('projects/project-123/locations/global/investigations/investigation-abc');
        expect(path.getRevisionName()).toBe('projects/project-123/locations/global/investigations/investigation-abc/revisions/revision-xyz');
    });

    it('Constructor with missing revision should work.', () => {
        const path = new InvestigationPath('project-456', 'investigation-def');
        expect(path.getInvestigationName()).toBe('projects/project-456/locations/global/investigations/investigation-def');
        expect(() => path.getRevisionName()).toThrow(/Investigation ID and\/or Revision ID are not set/);
    });
